...
public class ASTController <N>
{
	...
	public HashMap<String,List<Integer>> renamedURIHash(HashMap<TreeObject<N>, List<Integer>> astHash, String namespace){
		acmf.downloadSchemes();
		HashMap <String, List<Integer>> mapUriLines = new HashMap<>();
		//astList.remove(0);
		for(Map.Entry<TreeObject<N>, List<Integer>> entry: astHash.entrySet()) {
			
			String uri = acmf.getRenamedURI(namespace,entry.getKey().getNode().getClass());
			if(mapUriLines.containsKey(uri)) {
				List<Integer> lines =  mapUriLines.get(uri);
				List<Integer> oldList = entry.getValue();
				if(!lines.contains(oldList.get(0))){
					lines.add(oldList.get(0));
					Collections.sort(lines);
				}
			}else {
				List<Integer> lines = entry.getValue();
				//lines.add(line);
				mapUriLines.put(uri, lines);
